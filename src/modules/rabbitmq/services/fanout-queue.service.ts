...
export class FanoutQueueService {
   ...
   * 绑定队列到广播交换机
   */
  async bindQueueToFanout(exchangeName: string, queueName: string, options: QueueOptions = {}): Promise<void> {
    const fullExchangeName = `${this.fanoutExchangePrefix}.${exchangeName}`
   ...
   * 广播消息
   */
  async broadcast<T>(exchangeName: string, data: T, options: PublishOptions = {}): Promise<boolean> {
    const fullExchangeName = `${this.fanoutExchangePrefix}.${exchangeName}`
   ...
   * 消费广播队列
   */
  async consumeFanoutQueue<T>(queueName: string, handler: MessageHandler<T>): Promise<void> {
   ...
   * 适用于需要临时接收广播消息的场景
   */
  async createTemporaryFanoutQueue(exchangeName: string, handler: MessageHandler<any>): Promise<string> {
    const fullExchangeName = `${this.fanoutExchangePrefix}.${exchangeName}`
   ...
   * 批量绑定队列到广播交换机
   */
  async bindMultipleQueues(exchangeName: string, queueNames: string[], options: QueueOptions = {}): Promise<void> {
    const fullExchangeName = `${this.fanoutExchangePrefix}.${exchangeName}`
    ...
    // 批量绑定队列
    const bindPromises = queueNames.map((queueName) => this.bindQueueToFanout(exchangeName, queueName, options))

    await Promise.all(bindPromises)

    this.logger.log(`批量绑定 ${queueNames.length} 个队列到广播交换机 ${fullExchangeName}`)
  }
   ...
   * 解绑队列从广播交换机
   */
  async unbindQueueFromFanout(exchangeName: string, queueName: string): Promise<void> {
    const fullExchangeName = `${this.fanoutExchangePrefix}.${exchangeName}`
