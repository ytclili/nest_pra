...
export class PriorityQueueService {
  ...
  async setupPriorityQueue(
    ...
    this.logger.log(
      `优先级队列设置完成: ${fullQueueName}, 最大优先级: ${maxPriority}`,
    );
  }
  ...
  async sendPriorityMessage<T>(
    ...
    const result = await this.coreService.sendToQueue(
      fullQueueName,
      data,
      publishOptions,
    );
    ...
    this.logger.debug(
      `优先级消息发送到队列 ${fullQueueName}, 优先级: ${priority}`,
    );
   ...
   * 发送高优先级消息
   */
  async sendHighPriorityMessage<T>(
    queueName: string,
    data: T,
    options: PublishOptions = {},
  ): Promise<boolean> {
   ...
   * 发送中优先级消息
   */
  async sendMediumPriorityMessage<T>(
    queueName: string,
    data: T,
    options: PublishOptions = {},
  ): Promise<boolean> {
   ...
   * 发送低优先级消息
   */
  async sendLowPriorityMessage<T>(
    queueName: string,
    data: T,
    options: PublishOptions = {},
  ): Promise<boolean> {
  ...
  async sendBatchPriorityMessages<T>(
    ...
    this.logger.log(
      `批量发送 ${messages.length} 条优先级消息到队列 ${queueName}`,
    );
  ...
  async consumeTaskQueue(
    ...
    await this.consumePriorityQueue(
      'tasks',
      async (
        message: Message<{ type: string; data: any; createdAt: string }>,
      ) => {
