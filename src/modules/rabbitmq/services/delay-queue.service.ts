...
export class DelayQueueService {
   ...
   */
  async setupDelayQueue(queueName: string, options: DelayQueueOptions = {}): Promise<void> {
    const delayExchange = options.delayExchange || `${this.delayExchangePrefix}.${queueName}`
  ...
  async sendDelayMessage<T>(
    ...
    return this.coreService.publish(delayExchange, queueName, data, publishOptions)
  }

  /**
   * 消费延迟队列
   */
  async consumeDelayQueue<T>(queueName: string, handler: MessageHandler<T>): Promise<void> {
   ...
   * 取消延迟消息 (仅在使用TTL模式时有效)
   */
  async cancelDelayMessage(queueName: string, messageId: string): Promise<boolean> {
