...
export class RabbitMQInitService implements OnModuleInit {
  ...
  async initializeQueues(): Promise<void> {
    ...
    try {
      ...
      this.logger.log(`✅ 队列初始化完成！共设置 ${QUEUE_CONFIGS.length} 个队列`)
   ...
   * 手动初始化（用于API调用）
   */
  async manualInit(): Promise<{ success: boolean; message: string; queues: number }> {
    try {
      await this.initializeQueues()
      return {
        success: true,
  ...
  private groupByExchange(): Record<string, typeof QUEUE_CONFIGS> {
    return QUEUE_CONFIGS.reduce((groups, config) => {
   ...
   * 设置交换机和相关队列
   */
  private async setupExchangeAndQueues(exchangeName: string, configs: typeof QUEUE_CONFIGS): Promise<void> {
    ...
    this.logger.log(`✅ 交换机 ${exchangeName} 设置完成: ${configs.length} 个队列`)
  }
