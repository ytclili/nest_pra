...
export class ManualAckDemoService {
   ...
   * 启动手动ACK消费者
   */
  async startManualAckConsumer(options: {
    prefetch?: number
    workerId?: string
  } = {}): Promise<void> {
    ...
    await this.coreService.consumeWithManualAck(
      ...
      async (message: any, ackCallback) => {
        ...
        this.logger.log(`📨 [${workerId}] 开始处理消息: ${message.data.id}, 重试次数: ${retryCount}`)
        ...
        try {
          ...
          this.logger.log(`✅ [${workerId}] 消息处理成功: ${message.data.id}, 耗时: ${duration}ms`)

        } catch (error) {
          const duration = Date.now() - startTime
          this.logger.error(`❌ [${workerId}] 消息处理失败: ${message.data.id}, 耗时: ${duration}ms, 错误: ${error.message}`)

          // 检查重试次数
          if (retryCount < this.MAX_RETRIES) {
            // 重新入队，等待重试
            ackCallback.nack(true) // requeue = true
            this.logger.warn(`🔄 [${workerId}] 消息重新入队: ${message.data.id}, 重试次数: ${retryCount + 1}/${this.MAX_RETRIES}`)
          } else {
            // 超过最大重试次数，拒绝消息（进入死信队列）
            ackCallback.nack(false) // requeue = false，进入死信队列
            this.logger.error(`💀 [${workerId}] 消息超过最大重试次数，进入死信队列: ${message.data.id}`)
          }
        }
      },
      { prefetch }
   ...
   * 启动多个工作者
   */
  async startMultipleWorkers(workerCount: number = 3, prefetch: number = 1): Promise<void> {
    ...
    for (let i = 0; i < workerCount; i++) {
      ...
      workers.push(
        this.startManualAckConsumer({ prefetch, workerId })
      )
    }
   ...
   * 模拟业务处理逻辑
   */
  private async processBusinessLogic(messageData: any, workerId: string): Promise<void> {
  ...
  async getQueueStats(): Promise<{
    ...
    const [mainQueue, deadLetterStats] = await Promise.all([
      ...
      this.deadLetterService.getDeadLetterStats(this.DEMO_QUEUE).catch(() => null),
    ])
