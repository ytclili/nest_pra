...
export class WorkQueueService {
   ...
   * 设置工作队列
   */
  async setupWorkQueue(queueName: string, options: QueueOptions = {}): Promise<void> {
   ...
   * 发送工作任务
   */
  async sendWork<T>(queueName: string, data: T, options: PublishOptions = {}): Promise<boolean> {
    const fullQueueName = `${this.workQueuePrefix}.${queueName}`
   ...
   * 消费工作队列 (单个消费者)
   */
  async consumeWork<T>(queueName: string, handler: MessageHandler<T>, workerId?: string): Promise<void> {
    const fullQueueName = `${this.workQueuePrefix}.${queueName}`
    ...
    await this.coreService.consume(fullQueueName, async (message) => {
      ...
      try {
        ...
        this.logger.error(`工作者 ${id} 处理任务失败: ${message.id}, 错误: ${error.message}`)
        throw error
      }
    })
   ...
   * 批量发送工作任务
   */
  async sendBatchWork<T>(queueName: string, tasks: T[], options: PublishOptions = {}): Promise<boolean[]> {
    const sendPromises = tasks.map((task) => this.sendWork(queueName, task, options))
