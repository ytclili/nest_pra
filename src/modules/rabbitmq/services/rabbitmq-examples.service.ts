...
export class RabbitMQExamplesService {
   ...
   * - critical: 10
   */
  async sendPriorityTasks(tasks: Array<{ id: string; payload?: any; level: "low" | "normal" | "high" | "critical" }>) {
    ...
    const publishPromises = tasks.map((t) => {
      ...
      return this.core.publish("priority-exchange", "urgent", { id: t.id, level: t.level, payload: t.payload, ts: Date.now() }, { priority })
    })
   ...
   */
  async startPriorityConsumers(opts?: { workerCount?: number; prefetch?: number }) {
    const workerCount = Math.max(1, opts?.workerCount ?? 1)
    const prefetch = Math.max(1, opts?.prefetch ?? 1)

    this.logger.log(`启动优先级队列消费者：workers=${workerCount}, prefetch=${prefetch}`)

    await this.easy.startMultipleWorkers(
      "urgent-tasks",
      async (msg: { id: string; level: string; payload?: any; ts: number }) => {
        // 模拟处理：高优先级更快，低优先级更慢
        const base = msg.level === "critical" ? 100 : msg.level === "high" ? 200 : msg.level === "normal" ? 400 : 600
   ...
   * - 可以按需只启动某些队列
   */
  async startFanoutConsumers(opts?: { web?: boolean; mobile?: boolean; email?: boolean; prefetch?: number }) {
    const prefetch = Math.max(1, opts?.prefetch ?? 1)
    const toStart = {
      web: opts?.web ?? true,
    ...
    this.logger.log(`启动 Fanout 消费者：web=${toStart.web}, mobile=${toStart.mobile}, email=${toStart.email}, prefetch=${prefetch}`)

    const promises: Promise<void>[] = []

    if (toStart.web) {
      promises.push(
        this.easy.consumeQueue("notifications.web", async (n: any) => {
          // 模拟处理
    ...
    if (toStart.mobile) {
      promises.push(
        this.easy.consumeQueue("notifications.mobile", async (n: any) => {
          await new Promise((r) => setTimeout(r, 120))
          this.logger.log(`Mobile 通知: [${n.type}] ${n.title} - ${n.message}`)
        }, { prefetch })
      )
    }
    ...
    if (toStart.email) {
      promises.push(
        this.easy.consumeQueue("notifications.email", async (n: any) => {
          await new Promise((r) => setTimeout(r, 180))
          this.logger.log(`Email 通知: [${n.type}] ${n.title} - ${n.message}`)
        }, { prefetch })
      )
    }
   ...
   */
  async enqueueWork(count: number, template?: { subject?: string; content?: string }) {
    const jobs = Array.from({ length: count }).map((_, i) =>
      this.easy.sendEmail({
        to: `user${i + 1}@example.com`,
        subject: template?.subject ?? `Welcome #${i + 1}`,
        content: template?.content ?? `Hello user${i + 1}, this is a work-queue demo message.`,
      })
    )
  ...
  async startWorkConsumers(opts?: { workerCount?: number; prefetch?: number }) {
    ...
    this.logger.log(`启动工作队列消费者：workers=${workerCount}, prefetch=${prefetch}`)
