...
export class DeadLetterQueueService {
  ...
  async setupDeadLetterQueue(
    ...
    const dlxName =
      options.deadLetterExchange || `${this.dlxPrefix}.${originalQueue}`;
  ...
  async consumeWithRetry<T>(
    ...
    await this.coreService.consume(queueName, async (message: Message<T>) => {
      try {
        ...
        this.logger.error(
          `消息处理失败: ${message.id}, 错误: ${error.message}`,
        );
        ...
        if (retryCount < maxRetries) {
          // 创建重试消息
          try {
            const retryMessage = this.serializerService.createRetryMessage(
              message,
              maxRetries,
            );
            ...
            this.logger.log(
              `消息重试 ${retryMessage.retryCount}/${maxRetries}: ${message.id}`,
            );
          } catch (retryError) {
  ...
  async consumeDeadLetterQueue<T>(
    ...
    await this.coreService.consume(dlqName, async (message: Message<T>) => {
      ...
      try {
        ...
        this.logger.error(
          `死信消息处理失败: ${message.id}, 错误: ${error.message}`,
        );
   ...
   * 处理死信队列消息失败的情况
   */
  private async handleDeadLetterFailure<T>(
    message: Message<T>,
    error: Error,
  ): Promise<void> {
    // 记录到数据库或发送告警
  ...
  async setupMultipleDeadLetterQueues(
    ...
    const setupPromises = queues.map(({ name, options }) =>
      this.setupDeadLetterQueue(name, options),
    );
