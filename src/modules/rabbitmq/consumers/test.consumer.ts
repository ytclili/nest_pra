...
export class TestConsumer implements OnModuleInit {
  ...
  private async startTestConsumers(): Promise<void> {
    try {
      ...
      // 消费工作队列
      await this.rabbitMQService.consumeWork('test-work-queue', this.handleWorkMessage.bind(this), 'test-worker')

      // 消费延迟队列
      await this.rabbitMQService.consumeDelayQueue('test-delay-queue', this.handleDelayMessage.bind(this))

      // 消费优先级队列
      await this.rabbitMQService.consumePriorityQueue('test-priority-queue', this.handlePriorityMessage.bind(this))

      // 消费广播队列
      await this.rabbitMQService.consumeFanoutQueue('test-fanout-queue-1', this.handleFanoutMessage1.bind(this))
      await this.rabbitMQService.consumeFanoutQueue('test-fanout-queue-2', this.handleFanoutMessage2.bind(this))

      // 消费主题队列
      await this.rabbitMQService.consumeTopicQueue('test-topic-queue-1', this.handleTopicMessage1.bind(this))
      await this.rabbitMQService.consumeTopicQueue('test-topic-queue-2', this.handleTopicMessage2.bind(this))
      await this.rabbitMQService.consumeTopicQueue('test-topic-queue-all', this.handleTopicMessageAll.bind(this))
  ...
  private async handleDelayMessage(message: Message<any>): Promise<void> {
    ...
    this.logger.log(`📄 消息内容:`, message.data)
    this.logger.log(`⏱️ 消息创建时间: ${new Date(message.timestamp).toLocaleString()}`)
  ...
  private async handlePriorityMessage(message: Message<any>): Promise<void> {
    this.logger.log(`🔥 [优先级队列] 处理消息: ${message.id}, 优先级: ${message.priority || '未设置'}`)
    this.logger.log(`📄 消息内容:`, message.data)
