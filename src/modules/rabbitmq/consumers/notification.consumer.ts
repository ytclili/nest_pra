...
export class NotificationConsumer implements OnModuleInit {
  ...
  private async setupQueues(): Promise<void> {
    // 设置推送通知工作队列
    await this.rabbitMQService['workQueue'].setupWorkQueue(
      'push-notifications',
    );

    // 设置系统通知广播交换机
    await this.rabbitMQService['fanoutQueue'].setupFanoutExchange(
      'system-notifications',
    );

    // 绑定不同的通知队列到广播交换机
    await this.rabbitMQService.bindToFanout(
      'system-notifications',
      'notifications.web',
    );
    await this.rabbitMQService.bindToFanout(
  ...
  private async startConsumers(): Promise<void> {
    // 消费推送通知 (多个工作者)
    await this.rabbitMQService.startWorkers(
      'push-notifications',
    ...
    await this.rabbitMQService.consumeFanoutQueue(
      'notifications.web',
      this.handleWebNotification.bind(this),
    );

    // 消费移动端通知
    await this.rabbitMQService.consumeFanoutQueue(
      'notifications.mobile',
      this.handleMobileNotification.bind(this),
    );
  ...
  private async handlePushNotification(
    ...
    try {
      ...
      this.logger.error(
        `推送通知发送失败: ${message.id}, 错误: ${error.message}`,
      );
      throw error;
    }
  }
