...
export class TaskConsumer implements OnModuleInit {
  ...
  private async setupQueues(): Promise<void> {
    ...
    // 设置系统事件主题交换机
    await this.rabbitMQService["topicQueue"].setupTopicExchange("system-events")

    // 绑定不同类型的事件队列
    await this.rabbitMQService.bindToTopic("system-events", "events.user", "user.*")
    await this.rabbitMQService.bindToTopic("system-events", "events.order", "order.*")
    await this.rabbitMQService.bindToTopic("system-events", "events.system", "system.*")
  }
  ...
  private async startConsumers(): Promise<void> {
    // 消费定时任务
    await this.rabbitMQService.consumeDelayQueue("scheduled-tasks", this.handleScheduledTask.bind(this))

    // 消费用户事件
    await this.rabbitMQService.consumeTopicQueue("events.user", this.handleUserEvent.bind(this))

    // 消费订单事件
    await this.rabbitMQService.consumeTopicQueue("events.order", this.handleOrderEvent.bind(this))

    // 消费系统事件
    await this.rabbitMQService.consumeTopicQueue("events.system", this.handleSystemEvent.bind(this))
  }
  ...
  private async handleScheduledTask(
    ...
    try {
      ...
      this.logger.error(`定时任务执行失败: ${message.id}, 错误: ${error.message}`)
      throw error
    }
  }
