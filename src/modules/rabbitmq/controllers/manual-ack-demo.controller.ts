...
export class ManualAckDemoController {
  ...
  async startConsumer(@Body() body: { prefetch?: number; workerId?: string }) {
    try {
      ...
      // 异步启动消费者
      this.manualAckDemoService
        .startManualAckConsumer({ prefetch, workerId })
  ...
  async startMultipleWorkers(
    @Body() body: { workerCount?: number; prefetch?: number },
  ) {
    try {
      ...
      // 异步启动多个工作者
      this.manualAckDemoService
        .startMultipleWorkers(workerCount, prefetch)
        .catch((error) =>
          this.logger.error('多工作者运行异常:', error.message),
        );
  ...
  async startDeadLetterConsumer() {
    try {
      ...
      this.manualAckDemoService
        ...
        .catch((error) =>
          this.logger.error('死信队列消费者运行异常:', error.message),
        );
    ...
    try {
      ...
      setTimeout(() => {
        this.manualAckDemoService
          .startMultipleWorkers(workerCount, prefetch)
          .catch((error) =>
            this.logger.error('工作者运行异常:', error.message),
          );
      }, 1000);
