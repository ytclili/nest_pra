...
export class RabbitMQController {
  ...
  async sendTestMessage(
    ...
    switch (type) {
      ...
      case 'delay':
        return this.rabbitMQService.sendDelayMessage(
          queue!,
          data,
          delay || 5000,
        );
      ...
      case 'priority':
        return this.rabbitMQService.sendPriorityMessage(
          queue!,
          data,
          priority || 5,
        );
  ...
  async sendSMSTask(
    @Body() smsData: { phone: string; message: string; urgent?: boolean },
  ) {
  ...
  async publishEvent(@Body() eventData: { eventType: string; data: any }) {
    return this.rabbitMQService.publishEvent(
      eventData.eventType,
      eventData.data,
    );
  }
  ...
  async scheduleTask(
    @Body() taskData: { taskName: string; data: any; executeAt: string },
  ) {
